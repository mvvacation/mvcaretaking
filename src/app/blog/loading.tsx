export default function BlogLoading() {
  return (
    <>
      <section className="bg-navy-950 text-white section-padding relative overflow-hidden">
        <div className="container-narrow text-center">
          <div className="h-4 w-24 bg-navy-800 rounded mx-auto mb-4 animate-pulse" />
          <div className="h-10 w-80 bg-navy-800 rounded mx-auto mb-6 animate-pulse" />
          <div className="h-5 w-96 bg-navy-800/50 rounded mx-auto animate-pulse" />
        </div>
      </section>
      <section className="section-padding">
        <div className="container-narrow">
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {Array.from({ length: 6 }).map((_, i) => (
              <div key={i} className="rounded-2xl border border-navy-100/50 overflow-hidden">
                <div className="aspect-[16/9] bg-navy-100 animate-pulse" />
                <div className="p-6 space-y-3">
                  <div className="h-3 w-24 bg-navy-100 rounded animate-pulse" />
                  <div className="h-5 w-full bg-navy-100 rounded animate-pulse" />
                  <div className="h-4 w-3/4 bg-navy-50 rounded animate-pulse" />
                </div>
              </div>
            ))}
          </div>
        </div>
      </section>
    </>
  );
}
